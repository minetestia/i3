...
local function cache_groups(group, groups)
  ...
  for _, item in ipairs(items) do
    ...
    if def.drawtype and is_cube(def.drawtype) then texture = get_cube(tiles) end

    if texture then
      texture = texture:gsub("%^", "\\^"):gsub(":", "\\:")
        .. fmt("\\^[resize\\:%ux%u", px, px)
...
local function get_item_usages(item, recipe, added)
  if is_group(item) then
    ...
    if not group_cache then cache_groups(group, groups) end

    for name, def in pairs(reg_items) do
      if
        not added[name]
        and valid_item(def)
        and item_has_groups(def.groups, groups)
      then
...
local function cache_usages(item)
  ...
  if i3.fuel_cache[item] then
    i3.usages_cache[item] =
      table_merge(i3.usages_cache[item] or {}, { i3.fuel_cache[item] })
  end
end
...
local function drop_table(name, drop)
  ...
  for i = 1, #drop_items do
    ...
    if
      di.rarity
      or not max_items
      or (max_items and not di.rarity and count_sure < max_items)
    then
      for j = 1, #di.items do
    ...
    if not di.rarity then count_sure = count_sure + 1 end
  end
end
...
local function cache_recipes(item)
  ...
  if replacements[item] then
    ...
    for k, v in pairs(replacements[item]) do
      if _recipes[k] then _recipes[k].replacements = v end
    end
...
local function resolve_aliases(hash)
  for oldname, newname in pairs(reg_aliases) do
    ...
    if recipes then
      if not i3.recipes_cache[newname] then i3.recipes_cache[newname] = {} end
      ...
      for i = 1, #i3.recipes_cache[oldname] do
        ...
        if not similar then insert(i3.recipes_cache[newname], rcp_old) end
      end
    end

    if
      newname ~= ""
      and i3.recipes_cache[oldname]
      and reg_items[newname]
      and not hash[newname]
    then
...
local function init_cubes()
  for name, def in pairs(reg_nodes) do
    if def then
      ...
      elseif
        sub(def.drawtype, 1, 9) == "plantlike"
        or sub(def.drawtype, 1, 8) == "firelike"
      then
        local texture = true_str(def.inventory_image) and def.inventory_image
          or tiles[1]

        if texture then i3.plants[id] = texture end
      end
    end
  end
end
