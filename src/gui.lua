...
local function snip(str, limit, font_size)
  ...
  if utf8_len(str) > limit then return fmt("%s...", sub(str, 1, limit - 3)) end
end

local function get_desc(item, lang_code)
  if sub(item, 1, 1) == "_" then item = sub(item, 2) end

  local def = reg_items[item]
  if not def then return S("Unknown Item (@1)", item) end
  ...
  if true_str(desc) then
    ...
    if not find(desc, "%u") then desc = toupper(desc) end
...
local function get_stack_max(inv, data, is_recipe, rcp)
      ...
      for name in pairs(counts_rcp) do
        if is_group(name) then
          ...
          if def then
            ...
            local groups = group_cache and group_cache.groups
              or extract_groups(name)
  ...
  for _, count in pairs(counts) do
    if count < max_stacks then max_stacks = count end
  end
...
local function add_subtitle(fs, name, y, ctn_len, font_size, sep, label)
  ...
  if sep then image(0, y + 0.55, ctn_len, 0.035, PNG.bar) end
end

local function get_award_list(
  data,
  fs,
  ctn_len,
  yextra,
  ...
  local percent =
    fmt("%.1f%%", (awards_unlocked * 100) / award_list_nb):gsub("%.0", "")
  ...
  for i = 1, award_list_nb do
    ...
    if not award.unlocked then icon = fmt("%s^\\[colorize:#000:220", icon) end
...
local function get_isometric_view(fs, pos, X, Y, t, cubes, depth, high)
  ...
  for idx in area:iterp(pos1, pos2) do
    ...
    if img then
      ...
      if plant then size = size - 0.05 end
...
local function get_waypoint_fs(fs, data, player, yextra, ctn_len)
  ...
  for i, v in ipairs(data.waypoints) do
    ...
    fs(
      "style[%s;fgimg=%s;fgimg_hovered=%s;content_offset=0]",
      del,
      PNG.trash,
      PNG.trash_hover
    )
    ...
    fs(
      "style[%s;fgimg=%s;fgimg_hovered=%s;content_offset=0]",
      see,
      waypoint_preview and PNG.search_hover or PNG.search,
      PNG.search,
      PNG.search_hover
    )
    ...
    fs(
      "style[%s;fgimg=%s;content_offset=0]",
      vsb,
      v.hide and PNG.nonvisible or PNG.visible
    )
    image_button(ctn_len - 2, yi, icon_size, icon_size, "", vsb, "")
    fs(
      "tooltip[%s;%s;#32333899;#fff]",
      vsb,
      v.hide and ES "Show waypoint" or ES "Hide waypoint"
    )
...
local function get_bag_fs(fs, data, bag_size, yextra)
  ...
  fs(
    "style[bg_content;bgimg=%s;fgimg=i3_blank.png;bgimg_middle=10,%u;sound=]",
    PNG.bg_content,
    m
  )
  ...
  if not data.bag_rename then
    ...
    hypertext(
      ...
      "confirm_rename",
      fmt(
        "<global size=16><tag name=action color=#fff hovercolor=%s>"
          .. "<center><b><action name=ok>OK</action></b></center>",
        colors.yellow
      )
    )
  end
  ...
  fs(
    "list[detached:i3_bag_content_%s;main;%f,%f;4,%u;]",
    data.player_name,
    x,
    yextra + 1.3,
    bag_size
...
local function get_container(
  ...
  if damage_enabled then
    ...
    for i = 1, 10 do
      image(
        heart_x + ((i - 1) * (heart_size + 0.1)),
        heart_h,
        heart_size,
        heart_size,
        PNG.heart .. "^[colorize:#232428"
      )
    end
  ...
  for i, title in ipairs(i3.categories) do
    ...
    fs(
      "style[btn_%s;fgimg=%s;fgimg_hovered=%s;content_offset=0]",
      title,
  ...
  local function not_installed(modname)
    hypertext(
      ...
      "not_installed",
      fmt(
        "<global size=16><center><style color=%s font=mono>%s</style> not installed</center>",
        colors.blue,
        modname
      )
    )
  ...
  elseif data.subcat == 2 then
    if not i3.modules.armor then return not_installed "3d_armor" end
    ...
    for i = 1, 5 do
      ...
      if stack:is_empty() then
        local tips =
          { ES "Helmet", ES "Chest", ES "Leggings", ES "Boots", ES "Shield" }
  ...
  elseif data.subcat == 3 then
    if not i3.modules.skins then return not_installed "skinsdb" end
    ...
    for i = first, last do
      ...
      fs(
        [[ style[%s;padding=10;fgimg=%s;bgimg=%s;bgimg_hovered=i3_btn9_hovered.png;
  ...
  elseif data.subcat == 4 then
    if not i3.modules.awards then return not_installed "awards" end
...
local function show_settings(fs, data)
  ...
  elseif data.show_settings then
    ...
    fs(
      "style[setting_home:hovered;textcolor=%s]",
      show_home and colors.yellow or "#fff"
    )
    fs(
      "style[setting_style:hovered;textcolor=%s]",
      show_style and colors.yellow or "#fff"
    )
    fs(
      "style[setting_sorting:hovered;textcolor=%s]",
      show_sorting and colors.yellow or "#fff"
    )

    fs(
      "style[setting_home;bgimg=%s;bgimg_hovered=%s;bgimg_middle=9;padding=-9;textcolor=%s]",
      show_home and PNG.pagenum_hover or "",
      PNG.pagenum_hover,
      show_home and colors.yellow or "#ddd"
    )
    fs(
      "style[setting_style;bgimg=%s;bgimg_hovered=%s;bgimg_middle=9;padding=-9;textcolor=%s]",
      show_style and PNG.pagenum_hover or "",
      PNG.pagenum_hover,
      show_style and colors.yellow or "#ddd"
    )
    fs(
      "style[setting_sorting;bgimg=%s;bgimg_hovered=%s;bgimg_middle=9;padding=-9;textcolor=%s]",
      show_sorting and PNG.pagenum_hover or "",
      PNG.pagenum_hover,
    ...
    if show_home then
      ...
      if data.home then
        str = data.home
          ...
          :gsub("(%-?%d+)", function(a)
            c = c + 1
            return fmt(
              "<b>%s: <style color=%s font=mono>%s</style></b>",
              coords[c],
              colors.blue,
              a
            )
      ...
      hypertext(
        ...
        0.6,
        "home_pos",
        fmt("<global size=16><center>%s</center>", str)
      )
      fs(
        "style[set_home;padding=20,10,-210,-10;fgimg=%s;fgimg_hovered=%s]",
        PNG.home_px,
        PNG.home_px_hover
      )
    ...
    elseif show_style then
      ...
      checkbox(
        ...
        "cb_legacy_inventory",
        "Legacy inventory",
        tostring(data.legacy_inventory)
      )
      checkbox(
        2.6,
        10.85,
      ...
      if not recipe_filter_set() then
        checkbox(
          ...
          "cb_collapse",
          "Collapse list",
          tostring(data.collapse)
        )
      end

      local sign = (data.font_size > 0 and "+")
        or (data.font_size > 0 and "-")
        or ""
      label(5.3, 9.95, ES "Font size" .. fmt(": %s", sign .. data.font_size))

      local range = 8
      fs(
        "scrollbaroptions[min=-%u;max=%u;smallstep=1;largestep=1;thumbsize=2]",
        range,
        range
      )
      fs(
        "scrollbar[5.3,10.2;2.55,0.3;horizontal;sb_font_size;%d+]",
        data.font_size
      )

      fs(
        "tooltip[cb_hide_tabs;%s;#32333899;#fff]",
        ES "Enable this option to change the style of the right panel"
      )
      fs(
        "tooltip[cb_legacy_inventory;%s;#32333899;#fff]",
        ES "Enable this option to set the classic inventory size in Minetest"
      )
      fs(
        "tooltip[cb_wielditem_hud;%s;#32333899;#fff]",
        ES "Enable this option to show the wielded item description in your HUD"
      )
      fs(
        "tooltip[cb_collapse;%s;#32333899;#fff]",
    ...
    elseif show_sorting then
      ...
      fs(
        "dropdown[%f,%f;2.6,0.5;dd_sorting_method;%s;%u;true]",
        5.3,
        10.6,
        concat(methods, ","),
        data.sort
      ...
      if desc then tooltip(5.3, 10.6, 2.4, 0.5, ESC(desc)) end

      fs(
        "tooltip[cb_inv_compress;%s;#32333899;#fff]",
        ES "Enable this option to compress your inventory"
      )
      fs(
        "tooltip[cb_reverse_sorting;%s;#32333899;#fff]",
        ES "Enable this option to sort your inventory in reverse order"
      )
      fs(
        "tooltip[cb_ignore_hotbar;%s;#32333899;#fff]",
        ES "Enable this option to sort your inventory except the hotbar slots"
      )
      fs(
        "tooltip[cb_auto_sorting;%s;#32333899;#fff]",
...
local function get_slots(fs, data)
  ...
  fs(
    "style_type[list;size=%f;spacing=%f,%f]",
    size,
    spacing,
    legacy_inventory and 0.15 or spacing
  )

  fs(
    "list[current_player;main;%f,%f;%u,%u;%u]",
    inv_x,
    inv_y + (legacy_inventory and 1.25 or 1.15),
    hotbar_len,
    data.inv_size / hotbar_len,
    hotbar_len
  )
...
local function get_inventory_fs(player, data, fs)
  ...
  elseif i3.modules.awards and data.subcat == 4 then
    ...
    for i = 1, award_list_nb do
      local award = award_list[i]
      if award.unlocked then awards_unlocked = awards_unlocked + 1 end
    end
...
local function get_tooltip(item, info, lang_code)
  ...
  local function add(str) return fmt("%s\n%s", tooltip, str) end
  ...
  if info.repair then
    tooltip =
      add(S("Repairable by step of @1", clr("#ff0", i3.toolrepair .. "%")))
  end
    ...
    if several then
      for i = 1, #info.tools do
        names = fmt(
          "%s\t\t- %s\n",
          names,
          clr("#ff0", get_desc(info.tools[i], lang_code))
        )
      end

      tooltip =
        add(S("Only drop if using one of these tools: @1", sub(names, 1, -2)))
    else
      tooltip = add(
        S(
...
local function get_true_count(data, count, is_recipe, is_usage)
  ...
  if count_mul then count = count * count_mul end

  return count
end

local function get_output_fs(
  fs,
  data,
  rcp,
  is_recipe,
  ...
  if custom_recipe or shapeless or cooking then
    ...
    if tooltip then tooltip(pos_x, pos_y, 0.5, 0.5, ESC(tooltip)) end
  end
  ...
  if stackmax == 1 and count > 1 then label(X + 1.05, Y + 1, count) end
  ...
  local meta_desc = (short_desc ~= "" and short_desc)
    or (long_desc ~= "" and long_desc)
  ...
  if next(infos) then fs(get_tooltip(_name, infos, data.lang_code)) end
end
...
local function get_grid_fs(fs, data, rcp, is_recipe, is_usage)
  ...
  if large_recipe then fs "style_type[item_image_button;border=true]" end
  ...
  for i = 1, width * rows do
    ...
    if is_group(name) then
      ...
      name = group_cache and (group_cache.stereotype or group_cache.items[1])
        or groups_to_items(groups)[1]
        or ""
    end
    ...
    if count_sub == 2 then label = label:sub(2) end

    if not large_recipe then slot(X, Y, btn_size, btn_size) end
    ...
    if group_cache and group_cache.sprite and not large_recipe then
      ...
      if count > 1 then label(X + 0.8, Y + 0.9, count) end
    else
      item:set_name(name)
      item:set_count(count)
      local itemstr = ESC(item:to_string())
      ...
      local stackmax = item:get_stack_max()
      if stackmax == 1 and count > 1 then label(X + 0.95, Y + 0.95, count) end
    end
  ...
  if large_recipe then fs "style_type[item_image_button;border=false]" end
...
local function get_rcp_lbl(fs, data, panel, rn, is_recipe, is_usage)
  ...
  if is_recipe then lbl = ES("Recipe @1 of @2", data.rnum, rn) end

  local one = rn == 1
  local y = data.yoffset + 3.3

  hypertext(
    data.inv_width + (one and 4.7 or 3.95),
    y,
    3,
    0.6,
    "rcp_num",
  ...
  for _, v in ipairs(tiles) do
    ...
    elseif v.animation then
      _name = fmt(
        "%s^[verticalframe:%u:0",
        v.name,
        v.animation.frames_h or v.animation.aspect_h
      )
    end
...
local function get_header(fs, data)
  ...
  if nfavs < max_favs or (nfavs == max_favs and fav) then
    fs(
      "style[fav;fgimg=i3_fav%s.png;fgimg_hovered=i3_fav%s.png]",
      fav and "" or "_off",
      fav and "" or "_off"
    )
    image_button(star_x, star_y, size, size, "", "fav", "")
    fs(
      "tooltip[fav;%s;#32333899;#fff]",
      fav and ES "Unbookmark this item" or ES "Bookmark this item"
    )
  else
    fs(
      "style[nofav;fgimg=%s;fgimg_hovered=%s;fgimg_pressed=%s]",
      "i3_fav_off.png",
      PNG.cancel,
      PNG.cancel
    )
    ...
    fs(
      "tooltip[nofav;%s;#32333899;#fff]",
      ES "Unable to bookmark this item, limit reached"
    )
  end
  ...
  if _desc then tooltip(X, Y1 - 0.1, 5.7, 0.24, desc) end

  local _tech_name = snip(tech_name, name_lim, data.font_size)
  if _tech_name then tooltip(X, Y2 - 0.1, 5.7, 0.24, tech_name) end
...
local function get_crafting_fs(
  fs,
  data,
  is_recipe,
  is_usage,
  ...
  local show_crafting = (is_recipe and data.crafting_rcp)
    or (is_usage and data.crafting_usg)

  fs(
    "style[crafting_%s;fgimg=%s;fgimg_hovered=%s;content_offset=0]",
    name,
    fmt("%s", show_crafting and PNG.crafting_hover or PNG.crafting),
    PNG.crafting_hover
  )
  image_button(
    data.inv_width + 7.35,
  ...
  local stack_fs = (is_recipe and data.scrbar_rcp)
    or (is_usage and data.scrbar_usg)
    or 1
...
local function get_rcp_extra(fs, data, player, panel, is_recipe, is_usage)
  ...
  if rn then
    ...
    if rcp_ok then
      max_stacks_rcp, missing_rcp =
        get_stack_max(inv, data, is_recipe, panel.rcp[data.rnum])
    end

    if usg_ok then
      max_stacks_usg, missing_usg =
        get_stack_max(inv, data, is_recipe, panel.rcp[data.unum])
    end
    ...
    elseif rcp_ok or usg_ok then
      ...
      for item, count in pairs(missing) do
        if count > 0 then
          local name = is_group(item) and (i3.group_names[item:sub(7)] or item)
            or get_desc(item, data.lang_code)
      ...
      tooltip(
        ...
        0.45,
        0.45,
        fmt("%s:%s", ES "Missing materials for crafting", str)
      )
    end
    ...
    button(
      data.inv_width + 0.1,
      data.yoffset + (panel.height / 2) - 0.5,
      7.8,
      1,
      "no_rcp",
      lbl
...
local function hide_items(player, data)
  if compression_active(data) then
    ...
    for i = 1, #data.items do
      ...
      if not i3.compressed[item] then insert(new, item) end
    end
  ...
  then
    ...
    for i = 1, #data.items do
      ...
      if recipes or usages then insert(new, item) end
    end
...
local function get_header_items_fs(fs, data)
  ...
  if data.hide_tabs then
    fs(
      "style[enable_search;bgimg=%s;bgimg_hovered=%s;bgimg_pressed=%s]",
      data.enable_search and PNG.search_hover or PNG.search,
      PNG.search_hover,
      PNG.search_hover
    )
    ...
    if not recipe_filter_set() then
      for _, v in ipairs(i3.minitabs) do
        if v.name == "nodes" then insert(cat, { "node", "nodes only" }) end

        if v.name == "items" then insert(cat, { "item", "items only" }) end
      end
    end
    ...
    for i in ipairs(cat) do
      ...
      fs(
        "style[itab_%u;bgimg=%s;bgimg_hovered=%s;bgimg_pressed=%s;sound=i3_tab]",
        i,
        data.itab == i and active or PNG[name],
        active,
        active
      )
      image_button(
    ...
    if data.enable_search then
      ...
      if not true_str(data.filter) then
        image(
          ...
          4,
          0.01,
          PNG.search_outline_trim .. "^[opacity:100"
        )
      end
    else
  ...
  fs(
    "style[pagenum;bgimg=%s;bgimg_hovered=%s;bgimg_middle=9;padding=-9;sound=i3_click]",
    data.goto_page and PNG.pagenum_hover or "",
    PNG.pagenum_hover
  )
  ...
  if data.goto_page then
    ...
    fs(
      "style[goto_page;font=mono,bold;font_size=16;textcolor=%s]",
      colors.yellow
    )
...
local function get_minitabs(fs, data, player, full_height)
  ...
  for id, title in pairs(minitabs) do
    ...
    fs(
      [[ style[%s;bgimg=%s%s;bgimg_hovered=%s%s;noclip=true;font=bold;font_size=16;
    ...
    image_button(
      (data.inv_width - 0.65) + (X * (tab_len + 0.1)),
      top and -tab_hgh or full_height,
      tab_len,
      tab_hgh,
      "",
      tabname,
...
local function get_items_fs(fs, data, player, full_height)
  ...
  if data.pagenum > data.pagemax then data.pagenum = data.pagemax end
  ...
  if #items == 0 then
    ...
    for i = first_item, first_item + ipp - 1 do
      ...
      local item_btn =
        fmt("item_image_button", X, Y, size, size, name, item, "")
      ...
      if compressible(item, data) then
        ...
        fs(
          "tooltip[%s;%s;#32333899;#fff]",
          item,
          expand and ES "Click to hide" or ES "Click to expand"
        )
        fs "style_type[label;font=bold;font_size=20]"
...
local function get_favs(fs, data)
  ...
  for i, item in ipairs(data.favs) do
    ...
    if data.query_item == item then slot(X, Y, btn_size, btn_size) end
...
local function get_panels(fs, data, player)
  ...
  local usages =
    { name = "usages", rcp = data.usages, height = 4.045, func = get_rcp_extra }
  local panels = { title, recipes, usages, favs }
  data.yoffset = 0

  for i, panel in ipairs(panels) do
    if i > 1 then data.yoffset = data.yoffset + panels[i - 1].height + 0.1 end
...
local function get_tabs_fs(fs, player, data, full_height)
  ...
  for i, def in ipairs(i3.tabs) do
    if def.access and not def.access(player, data) then remove(_tabs, i) end
  end
  ...
  for i, def in ipairs(_tabs) do
    ...
    if not btm then shift = #_tabs - 3 end

    local selected = i == data.tab
    local bgimg = selected and (btm and PNG.tab_hover or PNG.tab_hover_top)
      or (btm and PNG.tab or PNG.tab_top)
    ...
    fs(
      [[ style[%s;bgimg=%s;bgimg_hovered=%s;bgimg_middle=%s;padding=%s;noclip=true;
    ...
    local X = (data.inv_width / 2)
      + (c * (tab_len + 0.1))
      - ((tab_len + 0.05) * (shift / 2))
    ...
    if true_str(def.image) then
      ...
      image(
        X + (tab_len / 2) - ((desc_len * 0.1) / 2) - 0.55,
        Y + 0.05,
        0.35,
        0.35,
        def.image
...
local function make_fs(player, data)
  ...
  fs(
    "formspec_version[%u]size[%f,%f]no_prepend[]bgcolor[#0000]",
    i3.settings.min_fs_version,
    data.inv_width + 8,
    full_height
  )
  ...
  if tab then
    if tab.formspec then tab.formspec(player, data, fs) end

    if tab.slots then get_slots(fs, data) end
  end
  ...
  if data.query_item then
    ...
    if not data.hide_tabs then get_minitabs(fs, data, player, full_height) end
  end
  ...
  if visible_tabs > 1 then get_tabs_fs(fs, player, data, full_height) end
