...
local get_bag_description, ItemStack =
  i3.get("get_bag_description", "ItemStack")
local S, ES, fmt, msg, slz, dslz =
  i3.get("S", "ES", "fmt", "msg", "slz", "dslz")
...
local function init_bags(player)
  ...
  local bag = create_inventory(fmt("i3_bag_%s", name), {
    allow_put = function(inv, _, _, stack)
      ...
      if empty and item_group > 0 and item_group <= 4 then return 1 end
  ...
  if data.bag then bag:set_list("main", get_content { data.bag }) end
  ...
  if data.bag then
    ...
    if content then bag_content:set_list("main", get_content(content)) end
  end
end
