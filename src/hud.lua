...
local function show_hud(player, data, notif, idx, dt)
  ...
  if notif.show then
    local speed = i3.settings.hud_speed
      * (100 * get_progress(offset.y, notif.max.y))
      * dt
    ...
    for _, def in pairs(notif.elems) do
      ...
      player:hud_change(def, "offset", {
        x = hud_info.offset.x,
        y = hud_info.offset.y
          - (speed * max(1, (#data.hud.notifs - idx + 1) / 1.45)),
      })
    end
  elseif
    notif.show == false and notif.hud_timer >= i3.settings.hud_timer_max
  then
    local speed = (i3.settings.hud_speed * 2.6)
      * (100 * get_progress(offset.x, notif.max.x))
      * dt
...
core.register_globalstep(function(dt)
  ...
  for i = 1, players[0] do
    ...
    for idx, notif in ipairs(data.hud.notifs) do
      if notif.show ~= nil then show_hud(player, data, notif, idx, dt) end
    end

    local has_text = player:hud_get(data.hud.wielditem).text ~= ""

    if not data.wielditem_hud then
      if has_text then player:hud_change(data.hud.wielditem, "text", "") end
      return
    end
...
local function init_waypoints(player)
  ...
  for _, v in ipairs(data.waypoints) do
    if not v.hide then
      local id =
        add_hud_waypoint(player, v.name, str_to_pos(v.pos), v.color, v.image)
      v.id = id
    end
  end
end
