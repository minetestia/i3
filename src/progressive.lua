...
i3.new_minitab("unlocked", {
  ...
  sorter = function(item, data) return data.items_progress[item] end,
})
...
local function item_in_inv(item, inv_items)
  ...
  if is_group(item) then
    ...
    for i = 1, inv_items_size do
      if inv_items[i] == item then return true end
    end
  end
end
...
local function progressive_filter(recipes, player)
  ...
  if #data.inv_items == 0 then return {} end
...
local function poll_new_items(player, data, join)
  ...
  if join or #diff > 0 then
    ...
    if data.discovered > 0 then
      local msg = fmt(
        "%u new recipe%s unlocked!",
        data.discovered,
        data.discovered > 1 and "s" or ""
      )
