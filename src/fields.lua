...
local function inv_fields(player, data, fields)
  ...
  for field in pairs(fields) do
    ...
    elseif sub(field, 1, 3) == "cb_" then
      ...
      if fields[field] == "true" then data[str] = true end
  ...
  if fields.quit then
    ...
    if data.filter == "" then data.enable_search = nil end
  ...
  elseif fields.confirm_rename then
    ...
    if fill then newname = fmt("%s (%s", newname, fill) end
  ...
  elseif fields.waypoint_add then
    ...
    if fields.waypoint_name == "" then waypoint = "Waypoint" end
    ...
    if idx and item ~= data.expand then
      ...
      if i3.compress_groups[item] then
        ...
        sort(items, function(a, b)
          if a:sub(1, 1) == "_" then a = a:sub(2) end
...
local function rcp_fields(player, data, fields)
  ...
  elseif
    fields.filter and (fields.key_enter_field == "filter" or fields.search)
  then
  ...
  elseif fields.prev_recipe or fields.next_recipe then
    ...
    data.rnum = data.recipes[num] and num
      or (fields.prev_recipe and #data.recipes or 1)
    data.crafting_rcp = nil
    data.scrbar_rcp = 1
  elseif fields.prev_usage or fields.next_usage then
    local num = data.unum + (fields.prev_usage and -1 or 1)
    data.unum = data.usages[num] and num
      or (fields.prev_usage and #data.usages or 1)
    data.crafting_usg = nil
    data.scrbar_usg = 1
  ...
  elseif fields.crafting_rcp or fields.crafting_usg then
    if fields.crafting_rcp then
      ...
      if not data.crafting_rcp then data.scrbar_rcp = 1 end
    else
      data.crafting_usg = not data.crafting_usg

      if not data.crafting_usg then data.scrbar_usg = 1 end
    end
  elseif
    (sb_rcp and sub(sb_rcp, 1, 3) == "CHG")
    or (sb_usg and sub(sb_usg, 1, 3) == "CHG")
  then
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
  ...
  if
    ...
    or fields.no_item
    or fields.no_rcp
    or fields.select_sorting
    or fields.sort_method
    or fields.bg_content
    or fields.quick_crafting
  then
    return false
  end
  ...
  if tab then
    if tab.slots then inv_fields(player, data, fields) end
