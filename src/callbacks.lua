...
minetest.register_on_player_inventory_action(function(player, _, _, info)
  ...
  if
    not minetest.is_creative_enabled(name)
    and (
      (info.from_list == "main" and info.to_list == "craft")
      or (info.from_list == "craft" and info.to_list == "main")
      or (info.from_list == "craftresult" and info.to_list == "main")
    )
  then
    set_fs(player)
  end
...
if minetest.global_exists "armor" then
  ...
  armor:register_on_equip(function(player, idx, stack)
    ...
    for i, v in ipairs(group_indexes) do
      ...
      if
        stackname:find "wood"
        or stackname:find "stone"
        or stackname:find "cactus"
      then
...
if minetest.global_exists "skins" then i3.modules.skins = true end

if minetest.global_exists "awards" then
  i3.modules.awards = true

  minetest.register_on_craft(function(_, player) set_fs(player) end)

  minetest.register_on_dignode(function(_, _, player) set_fs(player) end)

  minetest.register_on_placenode(function(_, _, player) set_fs(player) end)
...
local function get_lang_code(info) return info and info.lang_code end
...
local function outdated(name)
  minetest.show_formspec(
    name,
    "i3_outdated",
    ("size[6.5,1.3]image[0,0;1,1;i3_book.png]label[1,0;%s]button_exit[2.6,0.8;1,1;;OK]"):format "Your Minetest client is outdated.\nGet the latest version on minetest.net to play the game."
  )
end
...
insert(minetest.registered_on_joinplayers, 1, function(player)
  local name = player:get_player_name()
  local info = minetest.get_player_information
    and minetest.get_player_information(name)
