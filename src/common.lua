...
local function is_func(x) return type(x) == "function" end

local function true_str(str) return is_str(str) and str ~= "" end

local function true_table(x) return is_table(x) and next(x) end
    ...
    if search_filter then
      for filter_name, values in pairs(filters) do
        if values then
          ...
          to_add = (j > 1 and temp[item] or j == 1)
            and func(item, values)
            and (search_filter == "" or find(search_in, search_filter, 1, true))

          if to_add then temp[item] = true end
...
local function is_group(item) return sub(item, 1, 6) == "group:" end

local function extract_groups(str) return split(sub(str, 7), ",") end
...
local function get_group_stereotype(group)
  ...
  if valid_item(def) then return stereotype end
end
...
local function is_cube(drawtype)
  return drawtype == "normal"
    or drawtype == "liquid"
    or sub(drawtype, 1, 9) == "glasslike"
    or sub(drawtype, 1, 8) == "allfaces"
end
...
local function recipe_filter_set() return next(i3.recipe_filters) end
...
local function is_fav(data)
  for i = 1, #data.favs do
    if data.favs[i] == data.query_item then return i end
  end
end
...
local function sort_by_category(data)
  ...
  for i = 1, #items do
    ...
    if to_add then insert(new, item) end
  end
...
local function get_recipes(player, item)
  ...
  if recipes then recipes = apply_recipe_filters(recipes, player) end
  ...
  return not no_recipes and recipes or nil, not no_usages and usages or nil
end
...
local function craft_stack(player, data, craft_rcp)
  ...
  local rcp_def = rcp_usg == "recipe" and data.recipes[data.rnum]
    or data.usages[data.unum]
  local output = craft_rcp and data.recipes[data.rnum].output
    or data.usages[data.unum].output
  output = ItemStack(output)
  local stackname, stackcount, stackmax =
    output:get_name(), output:get_count(), output:get_stack_max()
        ...
        for _, pair in ipairs(rcp_def.replacements or {}) do
          ...
          if is_group(old_name) then
            ...
            for _, it in ipairs(item_groups) do
              if item == it then get_stack(player, ItemStack(new_name)) end
            end
...
local function sorter(inv, data, mode)
  sort(inv, function(a, b)
    ...
    if data.reverse_sorting then return a > b end
...
local function sort_inventory(player, data)
  ...
  list = data.inv_compress and compress_items(list, start_i)
    or pre_sorting(list, start_i)
...
local function reset_data(data)
  ...
  if data.itab > 1 then sort_by_category(data) end
end
  ...
  core.after(
    ...
    function()
      player:hud_set_hotbar_image(
        data.legacy_inventory and "gui_hotbar.png" or "i3_hotbar.png"
      )
    end
  )
end
