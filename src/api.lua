...
function i3.register_craft(def)
  ...
  if not is_table(def.items) then def.items = {} end

  if def.grid then
    if not is_table(def.grid) then def.grid = {} end

    if not is_table(def.key) then def.key = {} end
...
function i3.set_fs(player)
  ...
  if data.auto_sorting then sort_inventory(player, data) end
...
function i3.new_tab(name, def)
  ...
  elseif #i3.tabs == 6 then
    return err(
      fmt("i3.new_tab: cannot add '%s' tab. Limit reached (6).", def.name)
    )
  end
...
function i3.remove_tab(name)
  if not true_str(name) then return err "i3.remove_tab: tab name missing" end

  for i = #i3.tabs, 2, -1 do
    local def = i3.tabs[i]
    if def and name == def.name then remove(i3.tabs, i) end
  end
end
...
function i3.hud_notif(name, msg, img)
  ...
  local data = i3.data[name]
  if not data then return err "i3.hud_notif: no player data initialized" end
...
function i3.add_waypoint(name, def)
  ...
  if not data then return err "i3.add_waypoint: no player data initialized" end

  local player = get_player_by_name(def.player)
  local id = player
      and add_hud_waypoint(player, name, def.pos, def.color, def.image)
    or nil
  ...
  if data.subcat == 5 then data.scrbar_inv = data.scrbar_inv + 1000 end
...
function i3.remove_waypoint(player_name, name)
  ...
  for i = #data.waypoints, 1, -1 do
    ...
    if waypoint and name == waypoint.name then
      if player then player:hud_remove(waypoint.id) end
...
function i3.get_waypoints(player_name)
  ...
  local data = i3.data[player_name]
  if not data then return err "i3.get_waypoints: no player data initialized" end
...
function i3.remove_minitab(name)
  if not true_str(name) then return err "i3.remove_minitab: name missing" end

  for i = #i3.minitabs, 2, -1 do
    local v = i3.minitabs[i]
    if v and v.name == name then remove(i3.minitabs, i) end
  end
end
...
i3.new_minitab("all", {
  ...
  sorter = function() return true end,
})
